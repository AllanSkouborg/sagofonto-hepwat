package dk.artogis.hepwat.streamingagg.app;


import dk.artogis.hepwat.streamingagg.configuration.AggregationAndCalculations;
...
import dk.artogis.hepwat.streamingagg.streamingprocess.Aggregation;
...
public class application {
    ...
    private static AggregationAndCalculations aggregationAndCalculations;
        ...
        try
        {
            ...
            if (new String(loggingOn.toLowerCase()).equals(DefaultLoggingActivatedString.toLowerCase()))
            {
                ...
                try {
                    ...
                    RollingFileAppender rolingFileAppender = new RollingFileAppender(patternLayout, loggingPath + "StreamingAggregation_"+ fileName + ".log");
    ...
    private void startTimer(Logger logger)
    {
        TimerTask task = new TimerTask() {
            public void run() {
                ...
                if (localDeviceConfigurations != null)  {
                    if (logger.isInfoEnabled())
                        logger.info("tried to get device configuration");
        ...
        long delay =  serviceConfiguration.getConfiguration().getRecycleDataInterval();//1200000L;
        long periode = serviceConfiguration.getConfiguration().getRecycleDataInterval();//1200000L;
    ...
    public  application(URI serviceUri, Integer serviceId, Logger logger)
    {
        ...
        if ((serviceConfiguration != null)&&(serviceConfiguration.isOk())) {
            if(logger.isInfoEnabled())
                logger.info("configuration ok, going to get devicecalculations ");
            aggregationAndCalculations = new AggregationAndCalculations(serviceUri, serviceConfiguration.aggregationTypeId );
            if (aggregationAndCalculations.isOk) {
                ...
                if (deviceConfigurations != null) {
                    ...
                    aggregation = new Aggregation(serviceConfiguration, deviceConfigurations, aggregationAndCalculations.aggregationCalculationType, aggregationAndCalculations.aggregationInterval,  logger);
                    if (aggregation != null) {
                        startTimer(logger);
                        aggregation.run(lock);
                    } else logger.error("aggregation process not properly instantiated");
                } else logger.error("deviceCalculations not found");
            }
            else {
                System.out.println("aggregation and calculations from rest service not provided");
                logger.error("aggregation and calculations from rest service not provided");
            }
        }
        else {
