package dk.artogis.hepwat.calculationService.app;

import dk.artogis.hepwat.calculationService.configuration.*;

import dk.artogis.hepwat.calculationService.streamingprocess.Calculation;
...
public class application {
    ...
    private  DeviceConfigurationFormulas deviceConfigurationFormulas;
        ...
        try
        {
            ...
            if (new String(loggingOn.toLowerCase()).equals(DefaultLoggingActivatedString.toLowerCase()))
            {
                ...
                try {
                    ...
                    RollingFileAppender rolingFileAppender = new RollingFileAppender(patternLayout, loggingPath + "StreamingCalculation_"+ fileName + ".log");
    ...
    private void startTimer(Logger logger)
    {
        TimerTask task = new TimerTask() {
            public void run() {
                ...
                DeviceConfigurationFormulas localDeviceConfigurationFormulas = new DeviceConfigurationFormulas(serviceUri, serviceConfiguration.calcType, logger);
                if (localDeviceConfigurationFormulas != null) {
                    if (logger.isInfoEnabled())
                        logger.info("got device configuration");
                    if (localDeviceConfigurationFormulas.configurationFormulas != null) {
                        if (logger.isInfoEnabled())
                            logger.info("device configuration formulas not empty");
                        lock.lock();
                        try {
                            deviceConfigurationFormulas.configurationFormulas = localDeviceConfigurationFormulas.configurationFormulas;
                        } catch (Exception ex) {
    ...
    public application(URI  serviceUri, Integer serviceId, Logger logger)
    {
        ...
        if ((serviceConfiguration != null) && (serviceConfiguration.isOk())) {
            ...
            deviceConfigurationFormulas = new DeviceConfigurationFormulas(serviceUri, serviceConfiguration.calcType, logger);
            if (deviceConfigurationFormulas != null) {
                if (logger.isInfoEnabled())
                ...
                calculation = new Calculation(serviceConfiguration, deviceConfigurationFormulas, logger);
                if (calculation != null) {
                    startTimer(logger);
